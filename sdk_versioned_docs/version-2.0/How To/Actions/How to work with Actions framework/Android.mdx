...
context.installHumaSdk {
    sdk {
        actionKit {
            providers = listOf<KeyActionProvider>() // List of KeyActionProvider, empty by default
            datetimeRange =
                DateTimeRange() // DateTimeRange, by default range is -3 months to +14 days from today
            keyActionViewOverride = null // KeyActionViewOverride, null by default
        }
    }
    appkit {
        action {
            processors =
                listOf<ImportKeyActionProcessor>() // List of ImportKeyActionProcessor, empty by default
        }
    }
}
```
