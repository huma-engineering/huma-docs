...
When the processor is registered you can call `refreshKeyActions()` to fetch the configuration and fill your key action with a data.


```kt
HumaTrackManager.refreshKeyActions(datetimeRange: Pair<Instant, Instant>?) //call to refresh key actions
```
...
***Huma Track*** provides `KeyActionListView` that can show list of key actions cards and refresh it.
